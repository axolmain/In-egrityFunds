...
const SpendingTrendsChart: React.FC<SpendingTrendsChartProps> = ({ transactions, timePeriod }) => {
    const chartRef = useRef(null);

    useEffect(() => {
        // Prepare data by mapping transactions to { date, amount } format
        const spendingData: SpendingDataPoint[] = transactions.map((transaction) => ({
        ...
        const line = d3.line<SpendingDataPoint>()
    ...
    return (
        <div className="p-4 bg-white shadow-md rounded-md">
            <h3 className="text-lg font-semibold mb-4">Spending Trends ({timePeriod})</h3>
